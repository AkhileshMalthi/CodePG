...
from codepg.app import main, handle_config_command
...
def test_main_create_file(mock_open_editor, mock_create_file, temp_config_file, monkeypatch):
    ...
    # Mock sys.argv
    monkeypatch.setattr(sys, "argv", ["codepg", "create", "test.py", "--config", str(temp_config_file)])
...
def test_config_show_command(mock_config_class, capsys, mock_args):
    ...
    mock_config_instance.show_all.return_value = {"base_dir": "/test/path", "editor_command": "test {path}"}
